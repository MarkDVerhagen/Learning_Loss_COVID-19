# ---
# ROBUSTNESS: LIMITING THE ANALYSIS TO SCHOOLS WITH AT LEAST 75% TEST ATTENDANCE DURING THE PANDEMIC
# ---
...
#SPECIFY PATHS
tables_path <- "tables/supp/r_schools_75/"
...
# LIMIT DATA TO SCHOOLS WITH AT LEAST 75% TEST-ATTENDANCE DURING THE PANDEMIC

robust_ids <- function(df, var = "ALL", cutoff = 0.25) {
  ## Function identifies school_ids with < cutoff missing in both treat and control
  df$rel_var <- df[, var]
  treat <- df %>%
    filter(treat == 1)
  control <- df %>%
  ...
  gen_set <- function(subset, cutoff) {
    return(subset %>%
             group_by(school_id) %>%
             summarise(prop_miss = mean(is.na(rel_var))) %>%
             ungroup() %>%
             filter(prop_miss <= cutoff))  
  }
  treat_ids <- gen_set(treat, cutoff)
  control_ids <- gen_set(control, cutoff)
  
  return(unique(treat_ids$school_id[treat_ids$school_id %in% control_ids$school_id]))
}

rel_ids <- robust_ids(total_df, var = "ALL", cutoff = 0.25)
