# ---
# ROBUSTNESS: INCLUDING FAMILY FIXED EFFECTS
# ---
...
#SPECIFY PATHS
tables_path <- "tables/supp/r_sibling_FE/"
...
# Baseline model (LM)
fit <- lapply(dvs,
              FUN = function(x) lm(formula(paste(x, fun)), 
                                          data = total_df))
...
texreg(list(fit$ALL, fit$RW, fit$TBL, fit$SP), 
       ...
       custom.gof.rows = 
         list("Num. Groups (Family)" = c(length(fit$ALL$fixed_effects), length(fit$RW$fixed_effects),
                                         length(fit$TBL$fixed_effects), length(fit$SP$fixed_effects))),
       center = TRUE,
       include.ci = FALSE,
       label = "table:baseline", 
       file = paste0(tables_path, "ll_baseline_", 
                     paste(control_period, collapse = ""),
                     "_", treatment_period, ".tex"))


# Overall effect, Sibling FE
fit <- lapply(dvs,
              FUN = function(x) lm_robust(formula(paste(x, fun)), 
                                          clusters = school_id, se_type = "stata", 
                                          fixed_effects = family_id,
                                          data = total_df))
...
for(var in int_var) {
  ...
  texreg(list(fit$ALL, fit$RW, fit$TBL, fit$SP), 
         caption = paste0("Learning loss by ", var, "Baseline Model"), 
         custom.model.names = c("Composite", "Maths", "Reading", "Spelling"),
         custom.coef.map = var_names,
         center = TRUE,
         include.ci = FALSE,
         label = paste0("table", var),
         file = paste0(tables_path, "ll_", var, "_baseline_", 
                       paste(control_period, collapse = ""), "_", 
                       treatment_period, ".tex"))
