# ---
# ROBUSTNESS: USING ALL COMBINATION OF PREVIOUS YEARS AS ALTERNATIVE CONTROL GROUPS
# ---
...
#SPECIFY PATHS
tables_path <- "tables/supp/r_alt_control_years/"
...
for(control_period in control_periods) {
  ...
  # SPECIFY MODELS 
  if(length(control_period) > 1) {
    fun <- " ~ 1 + treat + year_s + days_between_all_s"
  } else {
    fun <- " ~ 1 + treat + days_between_all_s"
  }
...
# MAIN ANALYSIS -------------------------------------------------------------------------------
  ...
  for(var in int_var) {
    ...
    if(length(control_period) > 1) {
      ...
      fun <- paste0(" ~ 1 + treat * ", var, " + days_between_all_s")
    }
