# ---
# ROBUSTNESS: EVALUATE RESULTS INCLUDING ALL CONTROLS
# ---
...
#SPECIFY PATHS
tables_path <- "tables/supp/r_all_controls/"
...
# SPECIFY MODELS 
fun <- " ~ 1 + treat + year_s + days_between_all_s + class_year + female + ses + ability"
...
for(i in class_years) {
  fit_temp <- lapply(dvs,
                     FUN = function(x) lm_robust(formula(paste(x, gsub(" \\+ class_year", "", fun))), 
                                                 clusters = school_id, se_type = "stata", 
...
for(var in int_var) {
  ...
  # SPECIFY MODELS 
  fun <- paste0(" ~ 1 + treat * ", var, " + year_s + days_between_all_s + ses + female + class_year + ability")
